...
class CustomApplications(Applications):
    def app_default(self, c):
        ...
        if f.extension is not None:
            ...
            if f.extension in ('doc', 'docx'):
                return self.either(c, 'wps', 'libreoffice')
            if f.extension in ('ppt', 'pptx'):
                return self.either(c, 'wpp', 'libreoffice')
            if f.extension in ('xls', 'xlsx'):
                return self.either(c, 'et', 'libreoffice')
            if f.extension in ('odt', 'ods', 'odp', 'odf', 'odg', 'odb'):
...
CustomApplications.generic(
        ...
        'gmplayer', 'smplayer', 'vlc', 'wps', 'wpp', 'et',
        flags='d', deps=['X'])
